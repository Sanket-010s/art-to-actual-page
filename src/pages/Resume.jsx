...
const SkillBar = ({ name, level, delay }) => (<div className="mb-4">
    ...
    <div className="h-1.5 bg-skill-bar-track rounded-full overflow-hidden">
      <motion.div className="h-full bg-skill-bar rounded-full" initial={{ width: 0 }} animate={{ width: `${level}%` }} transition={{ duration: 1, delay }}/>
    </div>
  </div>);
const Resume = () => {
      ...
      <div className="container mx-auto px-8 py-16">
        <div className="grid md:grid-cols-3 gap-12">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            {softwareSkills.map((skill, i) => (<SkillBar key={skill.name} {...skill} delay={i * 0.1}/>))}

            <h3 className="font-display text-xl font-bold uppercase text-foreground mt-10 mb-6">
              Languages
            </h3>
            {languages.map((lang, i) => (<SkillBar key={lang.name} {...lang} delay={0.5 + i * 0.1}/>))}
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
            ...
            <div className="space-y-6">
              {experience.map((exp, i) => (<div key={i} className="flex gap-3 items-start">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }}>
            ...
            <ul className="space-y-1 mb-8">
              {whatCanIDo.map((item, i) => (<li key={i} className="text-muted-foreground text-sm font-body">
                  {item}
                </li>))}
            </ul>
            ...
            <ul className="space-y-1 mb-8">
              {designSkills.map((item, i) => (<li key={i} className="text-muted-foreground text-sm font-body">
                  {item}
                </li>))}
            </ul>
            ...
            <div className="flex flex-wrap gap-2">
              {hobbies.map((hobby) => (<span key={hobby} className="text-muted-foreground text-sm font-body">
                  {hobby}
                </span>))}
            </div>
          </motion.div>
